...
$getid=sprintf("SELECT count(entry_id) as daca_sunt  FROM permis where user_id=".$userid);
...
$getid=sprintf("SELECT max(entry_id) as max_entry  FROM permis where user_id=".$userid);
$rezult= $conn ->query($getid);
$entry_id =0;
if ($rezult->num_rows>0){
if($row = $rezult->fetch_assoc()) {
